#!/usr/bin/env bash
# setup-sunvox-scriptmodule.sh
#
# RetroPie Sunvox Engine "Emulator"
# A scriptmodule to install a Sunvox "emulator" for RetroPie.
#
# Author: hiulit
# Repository: https://github.com/hiulit/RetroPie-Sunvox
# License: MIT https://github.com/hiulit/RetroPie-Sunvox/blob/master/LICENSE
#
# Requirements:
# - RetroPie 4.x.x
...
readonly SCRIPT_TITLE="RetroPie Sunvox \"Emulator\""
readonly SCRIPT_DESCRIPTION="A scriptmodule to install a Sunvox \"emulator\" for RetroPie."

readonly SCRIPTMODULE_NAME="sunvox"
...
function install() {
    ...
    # Install the "sunvox" scriptmodule.
    ...
    if [[ "$?" -eq 0 ]]; then
        ...
        echo "Game extensions: .sunvox .zip."
        echo
    else
        echo >&2
...
function update() {
    ...
    # Update the "sunvox" scriptmodule.
    cat "$SCRIPT_DIR/$SCRIPTMODULE_FILE" >"$RPS_DIR/$SCRIPTMODULE_FILE"
...
function get_options() {
    if [[ -z "$1" ]]; then
        ...
        -v | --version)
            echo "$SCRIPT_VERSION"
            ;;
            #H -i, --install [path]     Installs the "sunvox" scriptmodule.
        ...
        -i | --install)
            ...
            install
            ;;
            #H -u, --uninstall [path]   Uninstalls the "sunvox" scriptmodule.
            #H                              Path: The location of the "RetroPie-Setup" folder.
            #H                              Default: "~/RetroPie-Setup".
        -u | --uninstall)
            check_path "$2"
            uninstall
            ;;
            #H -U, --update [path]      Updates the "sunvox" scriptmodule.
            #H                              Path: The location of the "RetroPie-Setup" folder.
            #H                              Default: "~/RetroPie-Setup".
        -U | --update)
            check_path "$2"
